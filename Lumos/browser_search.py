...
class BrowserSearcher:
    ...
    async def search_all(self, keyword: str = '') -> List[Dict]:
        ...
        # 每个平台独立创建和关闭浏览器，避免资源竞争
        if keyword:
            # 有关键词时，执行定向搜索
            for search_func in [self.search_bing_news, self.search_weibo, self.search_zhihu, self.search_36kr]:
                try:
                    # 每次搜索前完全重置浏览器状态
                    self.browser = None
                    self.context = None
                    ...
                    if results:
                        ...
                        logger.info(f"{search_func.__name__} 成功获取 {len(results)} 条")
                except Exception as e:
                    logger.error(f"搜索任务异常：{e}")
        else:
            # 无关键词时，获取各平台热榜
            # 优先微博（最稳定），然后尝试其他平台
            for search_func in [self.search_weibo, self.search_zhihu, self.search_36kr]:
                try:
                    # 每次搜索前完全重置浏览器状态
                    self.browser = None
                    self.context = None
                    ...
                    if results:
                        ...
                        logger.warning(f"{search_func.__name__} 返回空结果")
                except Exception as e:
                    logger.error(f"搜索任务异常：{e}")
            ...
            try:
                self.browser = None
                self.context = None
                results = await self.search_bing_news('热门新闻')
                if results:
                    all_results.extend(results)
            except Exception as e:
                logger.error(f"Bing 新闻搜索异常：{e}")
...
# 同步包装函数（用于 Flask 路由）
def sync_search(keyword: str = '', proxy: Optional[str] = None, timeout: int = 20) -> List[Dict]:
    """
    同步搜索接口（用于 Flask 路由）

    :param keyword: 搜索关键词
    :param proxy: 代理地址
    :param timeout: 超时时间（秒），默认 20 秒
    :return: 搜索结果列表
    """
    ...
    try:
        ...
        results = loop.run_until_complete(
            asyncio.wait_for(searcher.search_all(keyword), timeout=timeout)
        )
        return results
    except asyncio.TimeoutError:
        logger.warning(f"[浏览器搜索] 超时（{timeout}秒），返回空结果")
        return []
    finally:
        loop.close()
